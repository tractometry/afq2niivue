...
@app.cell
def _(mo):
    volume_browser = mo.ui.file_browser(
        initial_path=".",
        filetypes=[".gz", ".nii"],
        label="Select volume file (.nii.gz)",
        multiple=False,
    )
    ...
    mo.hstack([volume_browser, trx_browser])
    return volume_browser, trx_browser


@app.cell
def _(NiiVue, ShowRender, io, json, mo, np, zipfile, volume_browser, trx_browser):
    mo.stop(
        not volume_browser.value or not trx_browser.value,
        mo.md("**Select both files above to start the viewer.**")
    )

    final_volume = str(volume_browser.path())
